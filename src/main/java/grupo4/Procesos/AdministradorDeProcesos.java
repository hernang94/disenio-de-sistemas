...
private Runnable ejecutarProceso(Proceso proceso){
	...
	public void crearProcesoNuevo(Accion accion, LocalDateTime fechaEjecucion,long periodicidad){
		...
		if(periodicidad==0){
			scheduler.schedule(this.ejecutarProceso(proceso), ChronoUnit.SECONDS.between(LocalDateTime.now(), fechaEjecucion), TimeUnit.SECONDS );
		}
		else{
			scheduler.scheduleAtFixedRate(this.ejecutarProceso(proceso), ChronoUnit.SECONDS.between(LocalDateTime.now(),fechaEjecucion), periodicidad, TimeUnit.SECONDS );
		}
	}
}
