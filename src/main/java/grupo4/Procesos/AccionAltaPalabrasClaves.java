...
public AccionAltaPalabrasClaves(LocalComercialAdapter adapter,RepositorioDePois repositorio){
	...
	public void ejecutar() {
		...
		lista.stream().forEach(localExterno->repositorio.cambiarPalabrasClaves(localExterno.getNombre(), localExterno.getPalabrasClaves()));
		RepositorioDeResultadosDeEjecucion.getInstancia().agregarResultado(new ResultadosDeEjecucion(lista.size(), LocalDateTime.now(), "exito"));
	}
